...
//69,73,75,88，98，108，116,118,122,125，
...
//复习2
//1（有思路但写的不对），2（有思路但写的不对），3，5，7（溢出处理没注意），8（情况处理），10（不会），
//11（要仔细），13（一遍ac），14（一遍ac，简单），15（写的漏洞太多），17（不会），19（写的有漏洞）
//20(简单一遍ac)，21（写的不对），22（不会），23(没写出），26（一遍ac），28（kmp没写）
//29（没写出），33（面试题），34（仔细），36（不会），38（不会），42（没思路），43，44（不会）,45（不会），46（没一遍ac），
//49（map，sort用法不过关），55（写的不够好），69（没做出来），73（可以更简洁）,75(没做出）
//79(没有一遍写对），91（没一遍ac），92（仔细），98（没做出），99（morris遍历没看，以及指南上的进阶题），
//101（简单），103（简单），116（条件要判断好）
//，124（不会），125（简单），128（不会），138，141（一遍ac）142，167（简单要仔细），174（不会）
//200（BFS不会），206（一遍ac），208（不会），212（不会），230（进阶题不会），237（简单一遍ac），277,285，348,，
//486（不会），518（不会）543（要优化）
//628，653（简单要仔细），958（不会），1143（不会）


//比较难还要看的：28的kmp,53(进阶）,234o(1)空间，103bfs，72（空间优化）
...
//复习4
//144（一遍ac），137（不会）,136(一遍ac),260(没有一遍ac）

//复习 0628
//5(ok)，10(ok)，15（不太ok），20（没一遍ac），21（ok），23（不会），33（没一遍ac），46（没一遍ac），47（不太会），
//48（不太会），49（没一遍ac），60（不会），79(不太会），81（没一遍ac）
//50(不太会),53(ok),54（不会），58，93（不会），95（不会），96(不会），98(OK),
//100（一遍ac），105（没一遍ac），106，108（不会），109（不会），111（不会），114（不会）
//142，
