...
//29（没写出），33（面试题），34（仔细），36（不会），38（不会），42（没思路），43，44（不会）,46（没一遍ac），
//49（map，sort用法不过关），55（写的不够好），69（没做出来），73（可以更简洁）,75(没做出）
//79(没有一遍写对），91（没一遍ac），98（没做出），99（morris遍历没看，以及指南上的进阶题），
//101（简单），103（简单），116（条件要判断好）
//，124（不会），125（简单），，138，141（一遍ac）142，167（简单要仔细），174（不会）
//200（BFS不会），208（不会），212（不会），230（进阶题不会），237（简单一遍ac），277,285，348,，
